...
from p3d import resnet_v1_p3d
from slide_rec.data_preprocess import shot_data_prepare_optical_8
...
class ImgtoolkitVideo():
    def construct_network(self, frame_feat_input, batch_labels, weights, reuse, is_training):
        ...
        loss = cost + FLAGS.l2_weight * L2_frame
    ...
    def build_graph(self):
        self.batch_clips = tf.placeholder(tf.float32,
                                          [None, 8, CROP_SIZE, CROP_SIZE, CHANNEL_NUM],
                                          name='input')
...
def main(_):
    ...
    if FLAGS.job_name == "ps":
        ...
        with tf.train.MonitoredTrainingSession(master=server.target,
                                               ...
                                               hooks=hooks) as sess:
            sess.run(init_op)
            # tf.contrib.tfprof.ProfileContext(prof_dir, trace_steps=[], dump_steps=[]) as pctx:
            ...
            while step < FLAGS.train_steps:
                ...
                # TODO: data prepare
                # frames_batch, batch_labels, video_ids = shot_data_prepare(batch_data)
                start_1 = time.time()
                frames_batch, batch_labels, video_ids = shot_data_prepare_optical_8(batch_data)
                end_1 = time.time()
                feed_dict = {}
                ...
                if step % 100 == 0:
                    ...
                    for i in range(test_batch_num):
                        ...
                        frames_batch_test, batch_labels_test, video_ids_test = \
                            shot_data_prepare_optical_8(batch_data_test)
                    ...
                    print('script file:{}, dropout_rate:{}, l2_weight:{}'.format(os.path.basename(__file__), FLAGS.dropout_rate, FLAGS.l2_weight))
